...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Pro Tip (Zero-Cost Redemption):** To avoid fees and prevent credit forfeiture from canceled orders, use the "Pickup" option. For maximum safety, travel to the store, confirm your items are in stock, and only then place the mobile order for immediate pickup. This eliminates cancellation risk due to out-of-stock items, which would cause you to lose the monthly credit.'
WHERE "benefit_id" = 'csp_doordash_grocery';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Insider Tip (Maximize Cash Value):** Due to Marriott''s dynamic pricing, the night with the highest points cost may not be the night with the highest cash price. When booking a multi-night stay, analyze the nightly breakdown and apply your certificate to the night with the highest dollar value to maximize your savings, not necessarily the one costing the most points.'
WHERE "benefit_id" = 'brilliant_free_night_award';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Pro Move (Gift Card Conversion):** To liberate this credit from the restrictive resort list, find a participating resort that sells physical Hilton gift cards (e.g., the Hotel Del Coronado). Purchase a $200 gift card at the front desk using your Aspire card. Amex will see the qualifying resort charge and issue the credit, effectively converting the restricted credit into a flexible gift card usable at nearly any Hilton property worldwide.'
WHERE "benefit_id" = 'aspire_hilton_resort_credit';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Optimization 1 ("Sixth Night Free" Stack):** For a 6-night stay, book 5 nights with points to get the 5th night free, then book a separate reservation for the 6th night using your certificate. Contact the hotel to link the reservations for a seamless stay with two free nights.

**Credify Optimization 2 (Maximize Value):** Since the certificate is valid any night, check cash prices for your entire stay. Use the certificate on the most expensive night (e.g., a Tuesday in a business hub) to maximize its value, which may not be a weekend night.'
WHERE "benefit_id" = 'aspire_free_night';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Hack 1 (Takeout & Walk-In):** You don''t need a reservation. The credit triggers on any charge from a listed Resy restaurant. Use the Resy app as a directory, then place a takeout order or pay in person.

**Credify Hack 2 (Gift Card Conversion):** To bank the credit, purchase a digital gift card from a Resy partner restaurant that uses the Toast payment platform (toasttab.com). The charge codes as a direct restaurant purchase, triggering the credit and letting you use the value anytime.'
WHERE "benefit_id" = 'amex_gold_resy';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Banking Tip (App Reload Method):** To avoid losing the monthly credit, open the Dunkin'' app each month and reload your digital card with exactly $7 using your Gold card. This banks the value for later use. The accumulated balance can also be used at participating Baskin-Robbins locations.'
WHERE "benefit_id" = 'amex_gold_dunkin';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Reality Check:** This benefit is extremely limited. The list of participating restaurants is very small and concentrated in a few major cities. Before dining, you must verify the restaurant is on the specific "Exclusive Tables" list provided by Chase, not the broader Visa Dining Collection. No reservation is required; simply pay with your CSR at an eligible restaurant to trigger the credit.'
WHERE "benefit_id" = 'csr_dining_credit_h1' OR "benefit_id" = 'csr_dining_credit_h2';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Value Maximizer (Pickup Imperative):** To maximize value, place a pickup order. This avoids all delivery fees, service fees, and driver tips, ensuring your $5 credit is applied directly to the cost of your food and not service charges.'
WHERE "benefit_id" = 'csr_doordash_restaurant';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Gotcha Alert:** The credit is not valid for Lyft bike or scooter rentals. To receive the credit, you must pay for the ride directly with your CSR card stored in the app. Paying via a digital wallet like Apple Pay or PayPal, even if linked to your CSR, will make the ride ineligible for the credit.'
WHERE "benefit_id" = 'csr_lyft';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Compatibility Check:** This benefit provides free *individual* subscriptions. It is incompatible with and provides no value against existing Apple One or Apple Music Family plans. To use it, you would need to cancel your bundle and repurchase other services a la carte, which is impractical for most users.'
WHERE "benefit_id" = 'csr_apple_subscriptions';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Hack (United TravelBank Conversion):** The most reliable way to convert this incidental credit into a direct airfare subsidy is to load funds into a **United Airlines TravelBank** account. These transactions are consistently reimbursed, effectively turning your credit into a flexible fund for future United flights.'
WHERE "benefit_id" = 'boa_pre_airline_incidental';
...
UPDATE "public"."benefit_definitions"
SET "redemption_instructions" = '**Credify Simplicity Note (Effortless Redemption):** This "anti-hack" credit is designed for maximum simplicity. The list of eligible travel and dining merchants is so broad that the credit is consumed effortlessly through normal spending with no special effort or tracking required. Its value is in its certainty, effectively acting as an automatic reduction of the annual fee.'
WHERE "benefit_id" = 'usb_ar_travel_dining';